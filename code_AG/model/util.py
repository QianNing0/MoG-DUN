...
class Preprocessing(object):
    def __init__(self, scale, pca, para_input=10, kernel=21, noise=True, cuda=False, sig_min=0.2, sig_max=4.0,
                 rate_iso=1.0, scaling=3, rate_cln=0.2, noise_high=0.08, AT=False):
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        #torch.save(np.array(kernel_code.cpu()),'./pca_kernels.pth')
        # Down sample
        if self.cuda:
        ...
        if self.AT:
            if self.cuda:
                lr_blured_DT = b_GPUVar_Bicubic(lr_blured_t, 1/self.scale)
                lr_blured_AT = self.blur(Variable(lr_blured_DT).cuda(),b_kernels)
            else:
                lr_blured_DT = b_CPUVar_Bicubic(lr_blured_t, 1/self.scale)
                lr_blured_AT = self.blur(lr_blured_DT, b_kernels)

        # Noisy
        if self.noise:
            Noise_level = torch.FloatTensor(random_batch_noise(B, self.noise_high, self.rate_cln))
            lr_noised_t = b_GaussianNoising(lr_blured_AT, Noise_level)
        else:
            Noise_level = torch.zeros((B, 1))
            lr_noised_t = lr_blured_AT
